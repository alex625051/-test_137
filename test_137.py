...
def updateTable(data=[], usd=0.0):
    ...
    try:
        ...
        cursor.execute(f'SELECT "заказ №"  from {POSTGREStable}')
        current_rows = tuple([a[0] for a in cursor.fetchall()])
        new_rows = tuple([a[1] for a in data])
        cursor.execute(f'DELETE from {POSTGREStable} WHERE "заказ №" NOT IN {new_rows} ')
        for order in data:
            cursor.execute(f'DELETE from {POSTGREStable} WHERE "заказ №"= {order[1]} ')
        postgres_insert_query = f""" INSERT INTO {POSTGREStable} ("№", "заказ №", "стоимость" , "срок поставки","стоимость в руб.")
                                           VALUES (%s,%s,%s,%s,%s)"""
        cursor.executemany(postgres_insert_query, data)
        connection.commit()
        count = cursor.rowcount
        print(count, f"Данные успешно добавлены в таблицу {POSTGREStable}")
